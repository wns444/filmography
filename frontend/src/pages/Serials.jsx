  ...
  useEffect(() => {
    const fetchSerials = async () => {
      try {
        ...
        const res = await serialsAPI.search(query)
        setSerials(res.data)
      } catch (err) {
        setError('Ошибка загрузки сериалов')
        console.error(err)
      } finally {
        setLoading(false)
      }
    ...
    <div className="px-4 md:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">
          {searchQuery ? `Поиск: "${searchQuery}"` : 'Все сериалы'}
        </h1>
        <p className="text-gray-400">Найдено сериалов: {serials.length}</p>
      </div>
